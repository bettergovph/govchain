...
export default function DatasetCard({ dataset }: DatasetCardProps) {
  ...
  const normalizedDataset = {
    ...dataset, 
    pinCount: Number(dataset.pin_count),
    timestamp: Number(dataset.timestamp),
    file_size: Number(dataset.file_size),
  };

  const parsedData = useMemo(() => tryParseData(normalizedDataset.description), [normalizedDataset.description,]);
  const modes = [
  ...
  const downloadFile = () => {
    window.open(normalizedDataset.file_url, '_blank');
  };

  const viewOnBlockchain = () => {
    // Link to our blockchain explorer with the normalizedDataset index
    // In the explorer, we can look up the transaction by entry index
    window.open(`/explorer/tx/${normalizedDataset.tx_hash}`, '_blank');
  };
  ...
  return (
    <>
      <Card className="hover:shadow-md transition-shadow overflow-hidden">
        ...
        {isImageMimeType(normalizedDataset.mime_type) && (
          <div className="aspect-video relative overflow-hidden bg-gray-100">
            <img
              src={normalizedDataset.file_url}
              alt={normalizedDataset.title}
              className="w-full h-full object-cover hover:scale-105 transition-transform duration-200"
            // onError={(e) => {
            //   const target = e.target as HTMLImageElement;
            //   target.src = normalizedDataset.fallback_url || '/placeholder-image.png';
            // }}
            />
            ...
            <div className="absolute top-2 right-2">
              <Badge variant="secondary" className="bg-black/50 text-white border-none">
                ...
                {normalizedDataset.mime_type.split('/')[1].toUpperCase()}
              </Badge>
            </div>
        ...
        <CardHeader className="pb-4">
          <div className="flex items-start justify-between">
            ...
            {!isImageMimeType(normalizedDataset.mime_type) && (
              <div className="flex items-center gap-1 text-muted-foreground ml-4">
                {getFileIcon(normalizedDataset.mime_type)}
              </div>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            <Badge variant="secondary" className="gap-1">
              ...
              {normalizedDataset.agency}
            </Badge>
            <Badge variant="outline" className="gap-1">
              <Tag className="h-3 w-3" />
              {normalizedDataset.category}
            </Badge>
            {normalizedDataset.pinCount > 0 && (
              <Badge variant="default" className="gap-1">
                <Users className="h-3 w-3" />
                {normalizedDataset.pinCount} pins
              </Badge>
            )}
          </div>
        </CardHeader>
          ...
          <div className="flex items-center justify-between text-sm text-muted-foreground mb-4">
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1">
                ...
                {formatFileSize(normalizedDataset.file_size)}
              </span>
              <span className="flex items-center gap-1">
                <Clock className="h-3 w-3" />
                {formatDate(normalizedDataset.timestamp)}
              </span>
            </div>
          </div>

          <div className="flex gap-2">
            {isDownloadable(normalizedDataset.mime_type) && (
            ...
            <Button
              ...
              className={isDownloadable(normalizedDataset.mime_type) ? '' : 'flex-1'}
            >
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete normalizedDataset information and metadata
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              {/* <div className="grid grid-cols-3 gap-2 text-sm">
                  ...
                  <span className="col-span-2">{normalizedDataset.description}</span>
                </div> */}
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">Agency:</span>
                  <span className="col-span-2">{normalizedDataset.agency}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">Category:</span>
                  <span className="col-span-2">{normalizedDataset.category}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">Submitter:</span>
                  <span className="col-span-2">{normalizedDataset.submitter}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid gap-3">
                <div className="grid grid-cols-3 gap-2 text-sm">
                  ...
                  <span className="col-span-2 font-mono">{normalizedDataset.file_name}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">MIME Type:</span>
                  <span className="col-span-2 font-mono">{normalizedDataset.mime_type}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">File Size:</span>
                  <span className="col-span-2">{formatFileSize(normalizedDataset.file_size)}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">Uploaded:</span>
                  <span className="col-span-2">{new Date(normalizedDataset.timestamp).toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid gap-3">
                <div className="grid grid-cols-3 gap-2 text-sm">
                  ...
                  <span className="col-span-2 font-mono">{normalizedDataset.index}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">IPFS CID:</span>
                  <span className="col-span-2 font-mono break-all">{normalizedDataset.ipfs_cid}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">SHA-256:</span>
                  <span className="col-span-2 font-mono break-all">{normalizedDataset.checksum_sha_256}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <span className="font-medium">Pin Count:</span>
                  <span className="col-span-2">{normalizedDataset.pinCount} nodes</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <code className="flex-1 text-xs bg-muted p-2 rounded break-all">
                      {normalizedDataset.file_url}
                    </code>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => window.open(normalizedDataset.file_url, '_blank')}
                    >
                ...
                {normalizedDataset.fallback_url && (
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => window.open(normalizedDataset.fallback_url, '_blank')}
                      >
                        <ExternalLink className="h-3 w-3" />
                      </Button>
                    </div>
